...
struct s_sur_scr  { //: s_row
	...
	void drawCell(){
		...
		for(int i=0; i<numbers_cell; i++){
			...
			if(i==current_cell){
				GSetPixel16X(XCYCL(xc+1),YCYCL(yc+1),255);
				GSetPixel16X(XCYCL(xc-1),YCYCL(yc-1),255);
				GSetPixel16X(XCYCL(xc+1),YCYCL(yc-1),255);
				GSetPixel16X(XCYCL(xc-1),YCYCL(yc+1),255);
			}
			if(type==CELL_DIRECTED){
				...
				const int v_long=20;
				GLine16(xc,yc,XCYCL(xc+round(v_long*cos((float)alpha*3.1415926535/180))),YCYCL(yc-round(v_long*sin((float)alpha*3.1415926535/180))),255);
			}
			if(draw_seq){
					extern sqFont sysfont;
					XBuffer status;
					status <= i;
					sysfont.draw(Gx2x(XCYCL(xc+3)),Gy2y(XCYCL(yc + 3)),(unsigned char*)((char*)status),255,-1);
